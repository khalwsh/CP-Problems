...
/*==============================================  KHALWSH  ==============================================*/
int q(int mid,int n,int b){
    return ((mid * b + n % b))*(n/b - mid);
}
signed main() {
    khaled
    int t;
    ...
    while(t--){
        ...
        while(right - left >= 3){
            ...
            res =max(res ,max(q(mid1,n,b),q(mid2,n,b)));
            if(q(mid1,n,b)==q(mid2,n,b)){
        ...
        for(;left<=right;left++)res = max(res,q(left,n,b));
        cout<<res<<line;
    }
}