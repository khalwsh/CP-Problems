...
bool dfs_un(int node,int parent,vector<vector<int>>&v,int vis[])
{
    vis[node]=true;
    for(auto &val:v[node])
    {
        if(!vis[val])
            return dfs_un(val,node,v,vis);
        else if(parent!=val)
            return true;
    }
    return false;
}
bool dfs_di(int node,vector<vector<int>>&v,int vis[])
{
    vis[node]=1;
    for(auto &val:v[node])
    {
...
int n;
bool check_line_colomn(int a,int b)
{
    for(int i=0;i<n;i++)
    {
        if(b==i)
...
bool check_diagonale(int a,int b)
{
    for(int i=a-1,j=b-1;min(i,j)>=0;i--,j--)
    {
        if(v[i][j]=='Q')
            return false;
    }
    for(int i=a+1,j=b+1;max(i,j)<n;i++,j++)
    {
        if(v[i][j]=='Q')
            return false;
    }
    for(int i=a-1,j=b+1;j<n&&i>=0;i--,j++)
    {
        if(v[i][j]=='Q')
            return false;
    }
    for(int i=a+1,j=b-1;j>=0&&i<n;i++,j--)
    {
        if(v[i][j]=='Q')
            return false;
    }
...
long long max_sum2(vector<vector<char>>&ck,vector<vector<int>>&value)
{
    long long sum=0;
...
int main() {
    ...
    while(t--) {
        ...
        long long max_sum = 0;
        for (auto &val: res) {
            max_sum = max(max_sum, max_sum2(val, vvv));
        }
        cout <<setw(5)<<max_sum << line;
