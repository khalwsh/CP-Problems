...
bool solve(int index,vector<int>&v,int sum){
    if(index==v.size()){
        return sum==23;
    }
    return solve(index+1,v,sum+v[index])||solve(index+1,v,sum-v[index])||solve(index+1,v,sum*v[index]);
}
signed main() {
    khaled
    while(true){
        ...
        cout<<(flag?"Possible":"Impossible")<<line;
    }
}