...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
void dfs(int node,int parent,vector<int>adj[],vector<int>&low,vector<int>&high,int &timer,vector<pair<int,int>>&res,vector<bool>&vis){
    vis[node]=true;
    low[node]=high[node]=timer++;
    for(auto &val:adj[node]){
        if(val==parent)continue;
        if(!vis[val])dfs(val,node,adj,low,high,timer,res,vis),low[node]=min(low[val],low[node]);
        else low[node]=min(low[node],high[val]);
        if(low[val]>high[node])
            res.emplace_back(min(val,node),max(val,node));
    }
}
signed main() {
    khaled
    ...
    while(cin>>n){
        ...
        for(int i=0;i<n;i++){
            ...
            while(m--){
                int x;
                cin>>x;
                adj[par].emplace_back(x);
                adj[x].emplace_back(par);
            }
        }
        ...
        for(int i=0;i<n;i++){
            if(!vis[i])
                dfs(i,-1,adj,low,high,timer,temp,vis);
        }
        set<pair<int,int>>res(temp.begin(),temp.end());
        cout<<res.size()<<" critical links"<<line;
        for(auto &val:res)cout<<val.first<<" - "<<val.second<<line;
        cout<<line;
    }
}