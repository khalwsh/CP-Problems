...
const int max_size=2e5+9;
bool vis[max_size];
vector<vector<int>>vv(200000+8);
void dfs(int node)
{
    vis[node]=true;
    for(auto &val:vv[node])
    {
        if(!vis[val])
          dfs(val);
    }
...
int main() {
    ...
    while(left<right)
    {
        vv[v[left]].emplace_back(v[right]);
        vv[v[right]].emplace_back(v[left]);
        left++;
        right--;
    }
    int component=0;
