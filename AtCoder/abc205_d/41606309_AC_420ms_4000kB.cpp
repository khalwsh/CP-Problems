...
public:
    ...
    int diameter()//it's the length of the longest path form deep right to deep left
    {
...
unsigned long long binary_sea(vector<unsigned long long>&v,unsigned long long n,unsigned long long k)
{
    ...
    while(left<=right)
    {
        ...
        unsigned long long low= lower_bound(v.begin(),v.end(),mid)-v.begin();
        unsigned long long value= *lower_bound(v.begin(),v.end(),mid);
        if(value==mid)
        {
            if(mid-low<=k)
                left=mid+1;
...
int main() {
    ...
    /*
    binary_tree bin(-1);
    vector<int>path{2,3,4,5,6,7,9,99};
    vector<char>direction{'L','R','R','R','R','R','L','R'};
    bin.add(path,direction);
    direction[0]='R';
    path[0]=78;
    bin.add(path,direction);
    cout<<bin.diamater();*/
