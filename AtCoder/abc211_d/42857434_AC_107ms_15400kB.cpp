...
int BFS(vector<int>adj[],int n)
{
    ...
    queue<pair<int,int>>q;//node and freqency
    q.push({0,0});
    vector<pair<int,int>>distance(n,make_pair(INT_MAX,0));//distance and freq;
    distance[0]={0,1};
    while(!q.empty())
    {
        ...
        for(auto &val:adj[node])
        {
            if(distance[val].first>distance[node].first+1)
            {
                distance[val].first=distance[node].first+1;
                distance[val].second=distance[node].second;
                q.push({distance[val].first,val});
            }
            else if(distance[val].first==distance[node].first+1)
            {
                distance[val].second=distance[val].second%mod+distance[node].second%mod;
                distance[val].second%=mod;
            }
        }
    }
