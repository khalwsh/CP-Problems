...
bool cal_dist(vector<int>&v,vector<int>&a)
{
    long long res=0;
    ...
    if((double)sqrt(double(res))==(int)sqrt(res))
        return true;
    return false;
}
int main() {
    ...
    cin>>source>>target;
    source--,target--;
    vector<vector<long long>>dist(n, vector<long long>(3, INT64_MAX));//i will visit in three states the zero is the full ken_ken_pa
    dist[source][0] = 0;
    queue<pair<long long,long long>> q;
    q.push({source, 0});
    while (!q.empty()) {
        long long node = q.front().first;
        long long state = q.front().second;
        q.pop();
        for (int val : adj[node]) {
            if (dist[val][(state + 1) % 3] == INT64_MAX) {
                dist[val][(state + 1) % 3] = dist[node][state] + 1;
                q.push({val, (state + 1) % 3});
            }
        }
    }
    if (dist[target][0] == INT64_MAX)
        cout << -1 << endl;
    else
        cout << dist[target][0] / 3 << line;
