...
long long get_difference(multiset<int>&v)
{
    long long res=0;
    int n=(int)v.size();
...
int main() {
    ...
    int m;
    cin>>n>>m;
    vector<long long>child(n),teacher(m);
    for(auto &val:child)
        cin>>val;
    for(auto &val:teacher)
        cin>>val;
    sort(teacher.begin(),teacher.end());
    sort(child.begin(),child.end());
    vector<long long> sum1((n + 1) / 2), sum2((n + 1) / 2);
    for(int i = 0; i + 1 < n; i += 2)
        sum1[i / 2 + 1] = sum1[i / 2] + child[i + 1] - child[i];
    for(int i = n - 2; i > 0; i -= 2)
        sum2[i / 2] = sum2[i / 2 + 1] + child[i + 1] - child[i];
    long long ans = INT_MAX;
   
    for(auto & w : teacher){
        int x = int(lower_bound(child.begin(), child.end(), w) - child.begin());
        if(x & 1)
            x^=1;
        ans=min(ans, sum1[x / 2] + sum2[x / 2] + abs(child[x] - w));
    }
    cout << ans << endl;
}