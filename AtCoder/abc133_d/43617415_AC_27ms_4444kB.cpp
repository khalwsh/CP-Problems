...
int main() {
    ...
    for(auto &val:v)
        cin>>val;
    // (.5first+.5third) first (.5second+.5first) second (.5second+.5third) third
    // sum the even indexs and the odd with negative sign this is lead to the first dam or the last then build up on it
    ...
    //it is guranted that the value of sum is postive because n is odd and their is a unique res
    vector<long long>res(n);
    res[0]=summation;
    for(int i=n-1;i>0;i--)
    {
        res[i] = 2*v[i] - res[(i+1) % n];
    }
    for(auto &val:res)
        cout<<val<<" ";
}