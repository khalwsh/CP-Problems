...
int main() {
    ...
    for(int i=n-1;i>=0;i--)
    {
        res+=(long long)ma.size()-(long long)ma.count(v[i]);
        ma.insert(v[i]);
    }
    cout<<res<<line;*/
        int n;
        ...
        for(auto &val:v)
            cin>>val,repeat[val]++,summation+=val;
        int q;
        cin>>q;
        while(q--)
        {
            long long x,y;
            cin>>x>>y;
            cout<<summation-x*repeat[x]+y*repeat[x];
            cout<<line;
            summation=summation-x*repeat[x]+y*repeat[x];
            repeat[y]+=repeat[x];
            repeat[x]=0;
        }
}
