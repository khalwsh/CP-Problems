...
int solve(int index,int sum){
    ...
    if(~ret)return ret;
    if(s[index]!='?')return ret= solve(index-1,(sum%13+(powers[index]*(s[index]-'0'))%13)%13)%mod;
    int res=0;
    for(int i=0;i<=9;i++){
        res=add(res,solve(index-1,(sum%13+(powers[index]*(i))%13)%13),mod);
    }
    return ret=res;
}
signed main() {
    ...
    powers.resize(s.size());
    for(int i=s.size()-1;i>=0;i--){
        powers[i]=fast_power(10ll,(int)s.size()-(i+1),13);
    }

    cout<<solve(s.size()-1,0);
}