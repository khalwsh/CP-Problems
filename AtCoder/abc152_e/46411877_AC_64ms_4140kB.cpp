...
vector<int>factorize(int n){
    vector<int>res;
    for(int i=2;i*i<=n;i++){
        while(n%i==0)res.emplace_back(i),n/=i;
    }
...
signed main() {
    ...
    map<int,int>primes;
    for(auto &val:v){
        vector<int>temp= factorize(val);
        map<int,int>mp;
        for(auto &i:temp)mp[i]++;
        for(auto &i:mp)primes[i.first]=max(primes[i.first],i.second);
    }
    int cur=1;
    for(auto &val:primes){
        cur=mul(cur, fast_power(val.first,val.second,mod),mod);
    }
    int res=0;
    for(auto &val:v){
        vector<int>temp=factorize(val);
        ...
        int t=1;
        for(auto &i:mp){
           t=mul(t, fast_power(i.first,i.second,mod),mod);
        }
        res=add(res,mul(cur,inverse(t),mod),mod);
    }
    cout<<res<<line;
}
