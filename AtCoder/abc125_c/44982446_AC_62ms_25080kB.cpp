...
void build(int n,int k=25){
    for(int i=1;i<=k;i++){
        for(int j=0;j+(1<<i)<=n;j++){
            sparse_table[i][j]=__gcd(sparse_table[i-1][j],sparse_table[i-1][j+(1<<(i-1))]);
        }
    }
}
int query(int a,int b){
    int lg=(int)log2(b-a+1);
    return  __gcd(sparse_table[lg][a],sparse_table[lg][b-(1<<lg)+1]);
}
signed main() {
    khaled;
    int n;
    ...
    int mx=0;
    mx= query(1,n-1);
    mx=max(mx,query(0,n-2));
    for(int i=1;i<n-1;i++){
        mx=max(mx, __gcd(query(0,i-1), query(i+1,n-1)));
    }
    cout<<mx<<line;
}