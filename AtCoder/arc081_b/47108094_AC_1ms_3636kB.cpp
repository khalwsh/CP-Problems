...
string s1,s2;
int dp[55][4][4];
int solve(int index=0,int up=-1,int down=-1){
    //colors 0,1,2
    if(index==n){
        return 1;
    }
    int &ret=dp[index][up+1][down+1];
    if(~ret)return ret;
    int res=0;
    if(s1[index]==s1[index+1]){
        ...
        for(auto val:{0,1,2}){
            for(auto i:{0,1,2}){
                if(val!=i&&up!=val&&down!=i)
                    res=add(res,solve(index+2,val,i),mod);
            }
        }
    }else{
        //waef
        for(auto val:{0,1,2}){
            if(up!=val&&down!=val){
                res=add(res,solve(index+1,val,val),mod);
            }
        }
    }
    return ret= res;
