...
/*--------------------------------------------------------------------------------------------------------------------*/
int dp[N][2][101];
int solve(int index,string&s,bool tight,int d,int cur_mod){
    if(index==s.size()){
        return (cur_mod%d)==0;
    }
    int &ret=dp[index][tight][cur_mod];
    if(~ret)return ret;
    int res=0;
    int lim=(tight?9:s[index]-'0');
    for(int i=0;i<=lim;i++){
        res=add(res,solve(index+1,s,tight||s[index]-'0'>i,d,add(cur_mod,i,d)),mod);
    }
    return ret =  res;
}
signed main() {
    ...
    cout<<add(solve(0,k,false,d,0),-1,mod)<<line;
}