//#pragma GCC optimize("Ofast")
//#pragma GCC target("avx,avx2,fma")
#include<bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp> // Common file
#include <ext/pb_ds/tree_policy.hpp> // Including tree_order_statistics_node_update
using namespace __gnu_pbds;
using namespace std;
#define int long long
#define line '\n'
#define khaled ios_base::sync_with_stdio(0);cin.tie(0);cout.tie(0);
int dx[]{1,-1,0,0,1,1,-1,-1};
int dy[]{0,0,1,-1,1,-1,1,-1};
const long double pi=3.14159265350979323846;
const long double eps=1e-9;
template<typename T>
using ordered_set=tree<T,null_type,less_equal<T>,rb_tree_tag,tree_order_statistics_node_update>;
bool valid(long long i,long long j,long long n,long long m){return i>=0&&i<n&&j>=0&&j<m;}
long long mul(long long x,long long y,const long long&mod){return ((x%mod)*(y%mod))%mod;}
long long add(long long x,long long y,const long long&mod){return (((x%mod)+(y%mod))%mod+mod)%mod;}
long long fast_power(long long base,long long power,const long long &m=INT64_MAX){if (power == 1 || power == 0)return base * power + (!power);long long res = (fast_power(base, power / 2, m) % m) % m;if (power & 1)return mul(base,mul(res,res,m),m);else return mul(res,res,m);}
int log2_floor(long long i) {return i ? __builtin_clzll(1) - __builtin_clzll(i) : 0;}
int power_of_2(int n){ return __builtin_popcountll(n)==1;}
const int N=4e5+1;
//const int mod=1e9+7;
const int mod = 998244353;
const long long inf=2e17;
//__builtin_clzll(x): the number of zeros at the beginning of the number
//__builtin_ctzll(x): the number of zeros at the end of the number
//__builtin_popcountll(x): the number of ones in the number
//__builtin_parityll(x): the parity (even or odd) of the number of ones
/*=======================================================================================================*/
/*==============================================  KHALWSH  ==============================================*/
/*=======================================================================================================*/
int cnt=0;
pair<int,int>timing[N];
vector<int>adj[N];
void flatten(int node,int parent){
    timing[node].first=cnt++;
    for(auto &val:adj[node]){
        if(val==parent)continue;
        flatten(val,node);
    }
    timing[node].second=cnt++;
}
struct SegmentTree{
    int n;
    vector<int>tree,lazy;
    void init(int _n){
        n=_n;
        tree.resize(4*n);
        lazy.resize(4*n);
    }
    void prop(int node,int nl,int nr){
        if(lazy[node]!=0){
            tree[node]+=lazy[node];
            if(nl!=nr){
                lazy[2*node+1]+=lazy[node];
                lazy[2*node+2]+=lazy[node];
            }
            lazy[node]=0;
        }
    }
    void update(int node,int nl,int nr,int l,int r,int delta){
        prop(node,nl,nr);
        if(nl>r||nr<l)return;
        if(nl>=l&&nr<=r){
            tree[node]+=(nr-nl+1)*(delta);
            if(nl!=nr){
                lazy[2*node+1]+=delta;
                lazy[2*node+2]+=delta;
            }
            return;
        }
        int mid=nl+(nr-nl)/2;
        update(2*node+1,nl,mid,l,r,delta);
        update(2*node+2,mid+1,nr,l,r,delta);
        tree[node]=tree[2*node+1]+tree[2*node+2];
    }
    int query(int node,int nl,int nr,int l,int r){
        prop(node,nl,nr);
        if(nl>r||nr<l)return 0;
        if(nl>=l&&nr<=r)return tree[node];
        int mid=nl+(nr-nl)/2;
        return query(2*node+1,nl,mid,l,r)+query(2*node+2,mid+1,nr,l,r);
    }
};
bool contain(int a,int b){
    return timing[a].first<timing[b].first&&timing[a].second>timing[b].second;
}
signed main() {
    khaled
    int n;
    cin>>n;
    for(auto &val:timing)val= make_pair(-1,-1);
    vector<pair<int,int>>edges(n-1);
    for(int i=0;i<n-1;i++){
        int a,b;
        cin>>a>>b;
        a--,b--;
        adj[a].emplace_back(b);
        adj[b].emplace_back(a);
        edges[i]= make_pair(a,b);
    }
    flatten(0,-1);
    SegmentTree seg;
    seg.init(2*n);
    int q;
    cin>>q;
    while(q--){
        int type;
        int e,x;
        cin>>type>>e>>x;
        e--;
        if(type==1){
            int from,ex;
            from=edges[e].first;
            ex=edges[e].second;
            if(contain(ex,from)) {
                seg.update(0,0,2*n-1,timing[from].first,timing[from].second,x);
            }else{
                int start = timing[ex].first, ending = timing[ex].second;
                if (start - 1 >= 0)seg.update(0, 0, 2 * n - 1, 0, start - 1, x);
                if (ending + 1 < 2 * n)seg.update(0, 0, 2 * n - 1, ending + 1, 2 * n - 1, x);
            }
        }else{
            int from,ex;
            from=edges[e].first;
            ex=edges[e].second;
            if(contain(from,ex)){
                seg.update(0,0,2*n-1,timing[ex].first,timing[ex].second,x);
            }else{
                int start = timing[from].first, ending = timing[from].second;
                if (start - 1 >= 0)seg.update(0, 0, 2 * n - 1, 0, start - 1, x);
                if (ending + 1 < 2 * n)seg.update(0, 0, 2 * n - 1, ending + 1, 2 * n - 1, x);
            }
        }
    }
    for(int i=0;i<n;++i){
        cout<<seg.query(0,0,2*n-1,timing[i].first,timing[i].first)<<line;
    }
}
