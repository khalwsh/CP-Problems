...
signed main() {
    ...
    queue<pair<int,pair<int,int>>>q;//index from string,row,col;
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(valid(cur_row,cur_col,n,m)&&!vis[cur_row][cur_col]&&org[(cur_index+1)%(int)org.size()]==v[cur_row][cur_col]){
                q.push({(cur_index+1)%(int)org.size(),{cur_row,cur_col}});
                vis[cur_row][cur_col]=true;
            }
        }
    }
    cout<<(vis[n-1][m-1]?"Yes":"No")<<line;
