...
/*--------------------------------------------------------------------------------------------------------------------*/
int dp[3001][3001][2];
int solve(int i,int j,vector<int>&v,bool turn){
    if(i>j)return 0;
    if(dp[i][j][turn]!=-inf)return dp[i][j][turn];
    if(turn){
        int res1=solve(i+1,j,v,!turn)+v[i];
        int res2=solve(i,j-1,v,!turn)+v[j];
...
signed main() {
    ...
    int n;
    cin>>n;
    for(auto &val:dp)for(auto &i:val)for(auto&k:i)k=-inf;
    vector<int>v(n);
    for(auto &val:v)cin>>val;
    cout<<solve(0,n-1,v,true)<<line;
}