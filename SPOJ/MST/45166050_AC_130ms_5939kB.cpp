...
signed main() {
    ...
    for(auto &val:edges)cin>>val.second.first>>val.second.second>>val.first,val.second.first--,val.second.second--;
    sort(edges.begin(),edges.end());
    int cost=0;
    dsu d(n);
    for(auto &val:edges){
        if(d.find(val.second.first)==d.find(val.second.second))continue;
        cost+=val.first;
