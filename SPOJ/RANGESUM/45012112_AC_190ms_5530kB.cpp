...
signed main() {
    ...
    while(q--){
        ...
        if(type==1){
            int l,r;
            cin>>l>>r;
            cout<<fen.sum(r-1+fen.range_start+1)-fen.sum(l-1+fen.range_start)<<line;
        }else{
            int value;
            cin>>value;
            fen.add_element(value);
        }
    }
    cout<<line;
}