...
signed main() {
    ...
    int n,m,q;
    cin>>n>>m>>q;
    for(auto &val:result)val= make_pair(-1,-1);
        ...
        if(result[city].first==-1) {
            ...
            while (!pq.empty()) {
                ...
                for (auto &val: adj[node]) {
                    if (dist[node] + val.second < dist[val.first]) {
            ...
            result[city]= make_pair(big,cnt);
            cout<<result[city].first<<" "<<result[city].second<<line;
        }else{
            cout << result[city].first << " " << result[city].second << line;
        }
    }
