...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
vector<vector<int>>spare_table_min;
int k=25;
int n;
void build_table(){
    for(int i=1;i<k;i++){
        for(int j=0;j+(1<<i)<=n;j++){
            spare_table_min[i][j]=min(spare_table_min[i-1][j],spare_table_min[i-1][j+(1<<(i-1))]);
        }
    }
}
signed main() {
    ...
    spare_table_min.resize(k,vector<int>(n));
    for(int i=0;i<n;i++)cin>>spare_table_min[0][i];
    int q;
    build_table();
    cin>>q;
    while(q--){
        int l,r;
        cin>>l>>r;
        cout<<min(spare_table_min[lg[r-l+1]][l], spare_table_min[lg[r-l+1]][r-((1<<lg[(r-l+1)]))+1])<<line;
    }
}