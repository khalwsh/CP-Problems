...
struct Node{
    ...
    Node operator+(Node&v){
        Node node;
        int p1=0,p2=0;
        while(p1<values.size()&&p2<v.values.size()){
            if(values[p1]<v.values[p2])node.values.emplace_back(values[p1]),p1++;
            else node.values.emplace_back(v.values[p2]),p2++;
        }
        while(p1<values.size())node.values.emplace_back(values[p1]),p1++;
        while(p2<v.values.size())node.values.emplace_back(v.values[p2]),p2++;
        return node;
    }
};
class SegmentTree{
public:
    ...
    void build(int node,int nl,int nr,vector<int>&v){
        if(nl==nr){
            tree[node].values.emplace_back(v[nl]);
            return;
        }
        int mid=nl+(nr-nl)/2;
        build(1+2*node,nl,mid,v);
    ...
    int query(int node,int nl,int nr,int l,int r,int k){
        ...
        if(nl>=l&&nr<=r){
            return (int)tree[node].values.size()-
                   ( upper_bound(tree[node].values.begin(),tree[node].values.end(),k)
...
signed main() {
    ...
    while(q--){
        ...
        i--,j--;
        if(j<i)swap(i,j);
        cout<<seg.query(0,0,n-1,i,j,k)<<line;
    }
}