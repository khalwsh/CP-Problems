...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
int query(int a,int b,vector<int>&pref){
...
int solve(int l,int r,vector<int>&pref){
    if(l>=r)return 0;
    int &res=dp[l][r];
    if(vis[l][r]==tx)return res;
    vis[l][r]=tx;
    res=INT_MAX;
    for(int i=l;i<r;i++){
        int steps= query(i+1,r,pref)%100*(query(l,i,pref)%100);
        res=min(res,steps+solve(l,i,pref)+solve(i+1,r,pref));
    }
    return res;
}
signed main() {
