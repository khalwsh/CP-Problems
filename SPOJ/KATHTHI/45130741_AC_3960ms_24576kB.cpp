    ...
    while(t--){
        ...
        while(!q.empty()){
            ...
            for(int i=0;i<4;i++){
                int nr=row+dx[i];
                int nc=col+dy[i];
                if(valid(nr,nc,n,m)&&dist[row][col]+(v[nr][nc]!=v[row][col])<dist[nr][nc]){
                    dist[nr][nc]=dist[row][col]+(v[nr][nc]!=v[row][col]);
                    if(dist[nr][nc]==dist[row][col])
                        q.push_front({dist[nr][nc],{nr,nc}});
                    else
                        q.push_back({dist[nr][nc],{nr,nc}});
                }
            }
        }
        cout<<dist[n-1][m-1]<<line;
    }
}