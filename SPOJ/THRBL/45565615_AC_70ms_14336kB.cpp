...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
vector<vector<int>>spare_table;
int k=25;
void build(vector<int>&v){
    std::copy(v.begin(),v.end(),spare_table[0].begin());
    int n=(int)v.size();
    for(int i=1;i<=k;i++){
        for(int j=0;j+(1<<i)<=n;j++){
            spare_table[i][j]=max(spare_table[i-1][j],spare_table[i-1][j+(1<<(i-1))]);
        }
    }
}
int rangeMaxQuery(int L, int R) {
    ...
    int r = max(L, R);
    int p = log2(r - l + 1);
    return max(spare_table[p][l], spare_table[p][r + 1 - (1 << p)]);
}
signed main() {
    khaled
    int n,q;
    ...
    spare_table.resize(k+1,vector<int>(n));
    build(v);
    int cnt=0;
    while(q--){
        int L,R;
        ...
        if (L + 1 == R) {
            ...
            int maxHeight = rangeMaxQuery(L + 1, R - 1);
            if (maxHeight <= v[L]) {
                cnt++;
            }
        }
