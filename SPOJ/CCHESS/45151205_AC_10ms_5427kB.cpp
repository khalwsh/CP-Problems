...
signed main() {
    ...
    while(cin>>a>>b>>c>>d){
        ...
        while(!pq.empty()){
            ...
            for(int i=0;i<8;i++){
                int cur_row=row+dx2[i];
                int cur_col=col+dy2[i];
                if(valid(cur_row,cur_col,8,8)&&dist[cur_row][cur_col]>dist[row][col]+(row*cur_row+col*cur_col)){
                    dist[cur_row][cur_col]=dist[row][col]+(row*cur_row+col*cur_col);
