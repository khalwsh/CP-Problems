...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
void dfs(int node,int parent,vector<int>adj[],vector<int>&low,vector<int>&high,vector<bool>&mark,vector<bool>&vis,int timer){
    vis[node]=true;
    ...
    for(auto &val:adj[node]){
        ...
        if(!vis[val]){
            dfs(val,node,adj,low,high,mark,vis,timer);
            low[node]=min(low[val],low[node]);
            child++;
            if(low[val]>=high[node]&&parent!=-1)mark[node]=true;
        }else{
            low[node]=min(low[node],high[val]);
        }
    }
    if(parent==-1){
        if(child>1)mark[node]=true;
    }
}
int count_artifical_points(vector<int>adj[],int n){
    vector<int>low(n,0);
    vector<int>high(n,0);
    vector<bool>mark(n,false);
    vector<bool>vis(n,false);
    int timer=0;
    dfs(0,-1,adj,low,high,mark,vis,timer);
...
signed main() {
    ...
    while(true){
        ...
        cout<<count_artifical_points(adj,n)<<line;
    }
}