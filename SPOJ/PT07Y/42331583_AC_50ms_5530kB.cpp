...
bool BFS_cycle(int i,vector<vector<int>>&v,vector<int>&vis)
{
    vis[i]=true;
    ...
    while(!q.empty())
    {
        ...
        while(sz--)
        {
            ...
            for(auto &val:v[cur_node])
            {
                ...
                {
                    vis[val]=true;
                    q.push(make_pair(val,cur_node));
                }
                else if(cur_parent!=val)
                    return true;
            }
        }
    }
