...
void dfs2(int node,vector<bool>&vis,vector<int>adj[],vector<int>&last_scc){
    vis[node]=true;
    last_scc.emplace_back(node);
    for(auto &val:adj[node]){
        if(!vis[val])dfs2(val,vis,adj,last_scc);
    }
}
signed main() {
    khaled
    ...
    while(!finish_time.empty()){
        ...
        last_scc.clear();
        dfs2(node,vis,rev,last_scc);
    }
    cout<<last_scc.size()<<line;
    sort(last_scc.begin(),last_scc.end());
    for(auto &val:last_scc)cout<<val+1<<" ";
    cout<<line;
}
