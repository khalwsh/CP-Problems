...
int main() {
      ...
      temp.first=vp[0].first,temp.second=vp[0].second;
      for(long long i=0;i<n-1;i++)
      {
          if(temp.first<=vp[i+1].first&&temp.second>=vp[i+1].first)
          {
              temp.first=min(vp[i].first,min(vp[i+1].first,temp.first));
              temp.second=max(vp[i].second,max(temp.second,vp[i+1].second));
          }
          else {
              res.emplace_back(temp);
              temp.first=vp[i+1].first;
