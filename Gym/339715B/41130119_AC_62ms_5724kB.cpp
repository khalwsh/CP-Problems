...
bool binary_search_smaller_or_equal(vector<pair<long long,long long>>&vp,long long value1,long long value2);
int main() {
    khaled
    long long n,m;
    cin>>n>>m;
    ...
    vector<pair<long long,long long>>tail;
    long long one=vp[0].first;
    long long max_val_second=0;
    for(long long i=0;i<n-1;i++)
    {
        ...
        tail.emplace_back(one,vp[i].second);
        max_val_second=max(max_val_second,vp[i].second);
        one=vp[i+1].first;
    }
    tail.emplace_back(one,max(max_val_second,vp[vp.size()-1].second));
    ...
    while(q--)
    {
        long long l,r;
        cin>>l>>r;
        if(binary_search_smaller_or_equal(tail,l,r))
            cout<<"YES";
        else
            cout<<"NO";
        cout<<line;
...
{
    long long left=0;
    long long right=(long long)vp.size()-1;
    if(value1>value2)
        swap(value1,value2);
    while(left<=right)
    {
        long long mid=left+(right-left)/2;
        if(value1>=vp[mid].first) {
            left = mid + 1;
            if(vp[mid].second>=value2)
                return true;
        }
        else
          right=mid-1;
