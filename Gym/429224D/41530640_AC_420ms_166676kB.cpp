...
bool valid(vector<vector<int>>&pre,int mid,int left)
{
    if(!left) {
       return all_of(pre[mid].begin(),pre[mid].end(),compare);
    }
    else {
        for(int i=0;i<32;i++)
        {
            if(pre[mid][i]-pre[left-1][i]>1)
                return false;
        }
        return true;
    }
...
int main() {
    ...
    vector<vector<int>>prefix_bit(n,vector<int>(32));
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<32;j++)
        {
            if(!i)
                prefix_bit[i][j]=pre[i][j];
            else
                prefix_bit[i][j]=prefix_bit[i-1][j]+pre[i][j];
        }
    }
    /*for(auto &val:prefix_bit)
    {
        for(auto &i:val)
            cout<<i<<" ";
        cout<<line;
