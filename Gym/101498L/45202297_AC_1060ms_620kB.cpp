...
signed main() {
    ...
    while(t--){
        ...
        for(int i=0;i<m;i++){
            ...
            cin>>a>>b>>x;
            a--,b--;
            edges.emplace_back(a, make_pair(b,x));
            dist[b]=min(dist[b],x);
            mn=min(mn,x);
        }
        bool cycle=false;
        for(int i=0;i<n;i++) {
            for (auto &val: edges) {
                if(dist[val.second.first]>dist[val.first]+val.second.second){
                    if(i==n-1)cycle=true;
                    dist[val.second.first]=dist[val.first]+val.second.second;
                    mn=min(dist[val.second.first],mn);
                }
            }
        }
        for(int i=0;i<n;i++)mn=min(dist[i],mn);
        cout<<(cycle?"-inf": to_string(mn))<<line;
    }
}