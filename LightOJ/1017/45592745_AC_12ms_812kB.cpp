...
int vis[101][101];
int tx=1;
int solve(int row,int k,vector<pair<int,int>>&vp,int w){
    if(k==0)return 0;
    if(row==vp.size()){
        return 0;
    }
    if(vis[row][k]==tx)return dp[row][k];
    vis[row][k]=tx;
    int leave=solve(row+1,k,vp,w);
    int cost=0;
    int upper=vp[row].second+w;
    while(row<vp.size()&&vp[row].second<=upper)row++,cost++;
    int take=solve(row,k-1,vp,w)+cost;
    return dp[row-cost][k]= max(take,leave);
}
signed main() {
    khaled
    int t;
