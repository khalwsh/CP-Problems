...
int dp[51][1001];
int vis[51][1001];
int solve(int index,int k,vector<pair<int,int>>&vp){
    if(k<0)return 0;
    if(k==0)return 1;
    if(index==vp.size()){
        return k==0;
    }
    if(vis[index][k]==tx)return dp[index][k];
    vis[index][k]=tx;
    int temp=0;
    int res=0;
    while(temp<=vp[index].second){
        res=add(res,solve(index+1,k-temp*vp[index].first,vp),mod);
        temp++;
    }
...
signed main() {
    ...
    while(t--){
        ...
        for(auto &val:vp)cin>>val.first;
        for(auto &val:vp)cin>>val.second;
        cout<<"Case "<<tx++<<": "<<solve(0,k,vp)<<line;
    }
}