...
int dp[1001][2];
int vis[1001][2];
int solve(int index,bool building,vector<int>&t1,vector<int>&t2,vector<int>&one,vector<int>&two){
    if(index==t1.size()-1){
        if(building)return t2[index];
        else return t1[index];
    }
    if(vis[index][building]==tx)return dp[index][building];
    vis[index][building]=tx;
    if(building){
        // i am at building two
        int res=t2[index];
        return  dp[index][building]= res+min(solve(index+1,!building,t1,t2,one,two)+two[index],
        solve(index+1,building,t1,t2,one,two));
    }else{
        int res=t1[index];
        return  dp[index][building]= res+min(solve(index+1,!building,t1,t2,one,two)+one[index],
                       solve(index+1,building,t1,t2,one,two));
...
signed main() {
    ...
    while(t--){
        int n;
        cin>>n;
        vector<int>time_building_one(n),time_building_two(n);
        for(auto &val:time_building_one)cin>>val;
        for(auto &val:time_building_two)cin>>val;
        vector<int>one_to_two(n-1),two_to_one(n-1);
        for(auto &val:one_to_two)cin>>val;
        for(auto &val:two_to_one)cin>>val;
        cout<<"Case "<<tx++<<": ";
        cout<<min(solve(0,false,time_building_one,time_building_two,one_to_two,two_to_one),
                  solve(0,true,time_building_one,time_building_two,one_to_two,two_to_one))<<line;
    }
}