...
int dp[17][1<<16];
int vis[17][1<<16];
int solve(int row,int mask,vector<vector<int>>&adj){
    if(row==adj.size())return 0;
    int res=0;
    if(vis[row][mask]==tx)return dp[row][mask];
    vis[row][mask]=tx;
    for(int i=0;i<adj[0].size();i++){
        if(mask&(1<<i)){
            res=max(res,adj[row][i]+solve(row+1,mask^(1<<i),adj));
        }
    }
...
signed main() {
    ...
    while(t--){
        ...
        for(auto &val:adj)for(auto &i:val)cin>>i;
        cout<<"Case "<<tx++<<": ";
        cout<<solve(0,(1<<n)-1,adj)<<line;
    }
}