...
int vis[21][4];
int tx=1;
int solve(int row,vector<vector<int>>&v,int last_taken){
    if(row==v.size())return 0;
    int res=inf;
    int &ret=dp[row][last_taken];
    if(vis[row][last_taken]==tx)return ret;
    vis[row][last_taken]=tx;
    if(last_taken==0)res=min({solve(row+1,v,1)+v[row][1],solve(row+1,v,2)+v[row][2]});
    if(last_taken==1)res=min({solve(row+1,v,0)+v[row][0],solve(row+1,v,2)+v[row][2]});
    if(last_taken==2)res=min({solve(row+1,v,1)+v[row][1],solve(row+1,v,0)+v[row][0]});
    if(last_taken==3)res=min({solve(row+1,v,1)+v[row][1],solve(row+1,v,2)+v[row][2],solve(row+1,v,0)+v[row][0]});
    return ret = res;
}
signed main() {
    khaled
