...
int fn(int n) {
    if(~dp[n])return dp[n];
    if (n == 0) return dp[n]=a%mod;
    if (n == 1) return dp[n]=b%mod;
    if (n == 2) return dp[n]=c%mod;
    if (n == 3) return dp[n]=d%mod;
    if (n == 4) return dp[n]=e%mod;
    if (n == 5) return dp[n]=f%mod;
...
signed main() {
    ...
    for (int caseno = 1; caseno <= cases; ++caseno) {
        ...
        cout<<"Case "<<caseno<<": "<<fn(n)%mod<<line;
    }
    return 0;
}