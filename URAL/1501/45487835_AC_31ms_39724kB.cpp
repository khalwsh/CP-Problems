...
string s1,s2;
int dp[1001][1001][5];
bool solve(int i=0,int j=0,int diff=0){
    if(diff<-1||diff>1)return false;
    if(i==s1.size()&&j==s2.size()){
        return true;
    }
    if(~dp[i][j][diff+1])return dp[i][j][diff+1];
    bool move_first=false;
    if(i<s1.size()){
        move_first=solve(i+1,j,diff+(s1[i]=='1'?1:-1));
    }
    bool move_second=false;
    if(j<s2.size()){
        move_second=solve(i,j+1,diff+(s2[j]=='1'?1:-1));
    }
    return dp[i][j][diff+1]=move_first||move_second;
}
void build(int i=0,int j=0,int diff=0){
    ...
    if(i<s1.size()){
        move_first=solve(i+1,j,diff+(s1[i]=='1'?1:-1));
        if(dp[i][j][diff+1]==move_first){
            cout<<1;
            build(i+1,j,diff+(s1[i]=='1'?1:-1));
            return ;
        }
    }
    bool move_second=false;
    if(j<s2.size()){
        move_second=solve(i,j+1,diff+(s2[j]=='1'?1:-1));
        if(dp[i][j][diff+1]==move_second){
            cout<<2;
            build(i,j+1,diff+(s2[j]=='1'?1:-1));
            return ;
        }
    }
}
