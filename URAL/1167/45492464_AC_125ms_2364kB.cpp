...
signed main() {
    ...
    for(auto &val:dp)for(auto &i:val)i=inf;
    for(int i=0;i<=k;i++)dp[v.size()][i]=(i==0?0:inf);
    for(int i=(int)v.size()-1;i>=0;i--){
        for(int j=1;j<=k;j++){
            int cost=inf;
            for(int l=i;l<n;l++){
                cost=min(cost,dp[l+1][j-1]+query_sum(i,l,white_horse)* query_sum(i,l,black_horse));
            }
            dp[i][j]=cost;
        }
    }
