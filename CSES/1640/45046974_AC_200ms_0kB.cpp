...
int main() {
    ...
    cin>>summation;
    vector<int>v(n);
    map<int,pair<int,int>>mp;//first index second freqenc;
    for(int i=0;i<n;i++) {
        cin >> v[i];
        mp[v[i]].first=i+1,mp[v[i]].second++;
    }
    pair<int,int>res=make_pair(-1,-1);
    for(int i=0;i<n;i++)
    {
        if(mp[summation-v[i]].second>0)
        {
             if(i+1==mp[summation-v[i]].first)
                continue;
            res.first=i+1;
            res.second=mp[summation-v[i]].first;
            break;
        }
    }
