...
signed main() {
    ...
    for(auto &val:people)cin>>val;sort(people.begin(),people.end());
    for(auto &val:flat)cin>>val;sort(flat.begin(),flat.end());
    int p1=0,p2=0;
    int res=0;
    while(p1<n&&p2<m){
        if(people[p1]-k<=flat[p2]&&people[p1]+k>=flat[p2]){
            p1++,p2++;
            res++;
        }else if(people[p1]+k<flat[p2]){
            p1++;
        }else
            p2++;
    }
