...
int main() {
    khaled
    //the first n even numbers is n*n+2n
    //the first n odd numbers is n*n
    //so if we can get odd in a set and even in set then swap one from the even with one form the odd which their different is ceil(n/2)
    //7--->1,3,5,7
    //7--->2,4,6
    long long n;
    cin>>n;
    ...
    else
    {
        ...
        bool work=false;
        long long forbedin=-1;
        long long pos;
        for(long long k=n;k>=1;k--)
        {
            first.emplace_back(k);
            summation+=k;
            pos=k;
            if(target-summation<k)
            {
                if(target-summation==0)
                {
                    work=true;
                    break;
                }
                first.emplace_back(target-summation);
                forbedin=target-summation;
                work=true;
                break;
            }
        }
        if(work)
        {
            for(long long i=pos-1;i>=1;i--) { if(i!=forbedin)second.emplace_back(i); }
            cout<<"YES";
            cout<<" ";
            cout<<first.size()<<" ";
            for(auto &val:first)cout<<val<<" ";
            cout<<second.size();
            cout<<" ";
