...
vector<pair<int,bool>>seive(long long n)
{
    vector<pair<int,bool>>prime(n+1, make_pair(1,true));
    prime[0].second=prime[1].second=false;
...
int main() {
    ...
    cin>>a>>b>>c;
    long long res=0;
    vector<pair<int,bool>>seivee=seive(1000003);
    for(int i=1;i<=a;i++)
    {
        for(int j=1;j<=b;j++)
        {
            for(int k=1;k<=c;k++)
            {
               res=(res%mod+seivee[i*j*k].first%mod) %mod;
            }
        }
    }
    cout<<res<<line;
