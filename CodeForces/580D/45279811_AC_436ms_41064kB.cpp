...
/*--------------------------------------------------------------------------------------------------------------------*/
int dp[20][1<<18];
int solve(int mask,int m,vector<int>&v,vector<vector<int>>&adj_matrix,int last_taken=19){
    if(m==0){
        return 0;
    }
    int &res=dp[last_taken][mask];
    if(~res)return res;
    res=0;
    for(int i=0;i<v.size();i++){
        if(mask&(1<<i)){
            int bef=0;
            if(last_taken!=19){
                bef+=adj_matrix[last_taken][i];
            }
            res=max(res,solve(mask^(1<<i),m-1,v,adj_matrix,i)+v[i]+bef);
        }
    }
    return res;
}
signed main() {
    ...
    cout<<solve((1<<n)-1,m,v,adj_matrix)<<line;
}