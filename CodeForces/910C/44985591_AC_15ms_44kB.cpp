...
signed main() {
    ...
    set<char>forbidden_zero;
    for(auto &val:s){
        forbidden_zero.insert(val[0]);
    }
    for(int i=0;i<n;i++){
        reverse(s[i].begin(),s[i].end());
        int c=0;
        for(int j=0;j<s[i].size();j++){
            factors[s[i][j]-'a']+= fast_power(10ll,c);
            c++;
        }
    }
...
//    cout<<line;
    int res=0;
    for(int i=0;i<=9;i++){
        int ind= max_element(factors.begin(),factors.end())-factors.begin();
        if(factors[ind]==0)break;
        if(forbidden_zero.find((char)(ind+'a'))!=forbidden_zero.end()){
            auto it=av.begin();
            if(*it==0)it++;
            res+= factors[ind]**it;
            av.erase(it);
            //cout<<(char)(ind+'a')<<" "<<*it<<" "<<factors[ind]**it<<line;
            factors[ind]=0;
        }else{
            auto it=av.begin();
            res+= factors[ind]**it;
            av.erase(it);
            //cout<<(char)(ind+'a')<<" "<<factors[ind]**it<<line;
            factors[ind]=0;
        }
    }
    cout<<res<<line;
