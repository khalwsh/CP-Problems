...
int solve(int n1,int n2,int k1,int k2,int order1,int order2){
    if(order1>k1||order2>k2||n1<0||n2<0)return 0;
    if(n1==0&&n2==0){
        return 1;
    }
    int &res=dp[n1][n2][order1][order2];
    if(~res)return res;
    return res= add( solve(n1-1,n2,k1,k2,order1+1,0),solve(n1,n2-1,k1,k2,0,order2+1),mod);
}
signed main() {
    khaled
    int n1,n2,k1,k2;
    ...
    cout<<solve(n1,n2,k1,k2,0,0)<<line;
}