...
int main() {
    ...
    int q;
    cin>>q;
    vector<int>summation(200005,0);
    for(auto &val:v)
    {
        cin>>val.first>>val.second;
        summation[val.first]+=1;
        summation[val.second+1]-=1;
    }
    v.clear();
    vector<int>other(200005,0);
    other[0]=summation[0];
    for(int i=1;i<200005;i++)
    {
        other[i]=summation[i]+other[i-1];

    }
    summation.clear();
    vector<int>last_one(200005,0);
    for(int i=0;i<200005;i++)
        if(other[i]>=k)
            last_one[i]=1;
    other.clear();
    vector<int>pre_last(200005,0);
    pre_last[0]=last_one[0];
    for(int i=1;i<200005;i++)
    {
        pre_last[i]=pre_last[i-1]+last_one[i];
