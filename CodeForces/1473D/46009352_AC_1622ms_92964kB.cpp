...
void C(){
    ...
    if(l==1){
        ...
        cout<<*max_element(v.begin(),v.end())<<line;
}
...
signed main() {
    ...
    while(t--){
        ...
        vector<vector<int>>sparse_mn(k+1,vector<int>(n)),sparse_mx(k+1,vector<int>(n));
        copy(pre.begin(),pre.end(),sparse_mn[0].begin());
        copy(pre.begin(),pre.end(),sparse_mx[0].begin());
        for(int i=1;i<=k;i++){
            for(int j=0;j+(1<<i)<=n;j++){
                sparse_mn[i][j]=min(sparse_mn[i-1][j], sparse_mn[i-1][j+(1<<(i-1))]);
            }
        }
        for(int i=1;i<=k;i++){
            for(int j=0;j+(1<<i)<=n;j++){
                sparse_mx[i][j]=max(sparse_mx[i-1][j], sparse_mx[i-1][j+(1<<(i-1))]);
            }
        }
        while(q--){
            int l,r;
            cin>>l>>r;
            l--,r--;
            int mn1= query(0,l-1,sparse_mn,n,false);
            int mx1= query(0,l-1,sparse_mx,n,true);
            int mn2= query(r+1,n-1,sparse_mn,n,false);
            int mx2=query(r+1,n-1,sparse_mx,n,true);
            int start=query(l-1,l-1,sparse_mn,n,false)-query(r,r,sparse_mn,n,false);
            if(r!=n-1)
                mn2+=start,mx2+=start;
            int mx=max({mx1,mx2,0ll});
            int mn=min({mn1,mn2,0ll});
            cout<<mx+abs(mn)+1<<line;
        }
    }
}