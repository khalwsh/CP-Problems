...
private:
    ...
    int length=0;
    int cal_height(binary_tree*temp)
    {
        int h=1;
        if(temp&&temp->left)
            h=std::max(h,1+ cal_height(temp->left));
        if(temp&&temp->right)
            h=std::max(h,1+ cal_height(temp->right));
        return h;
    }
public:
    ...
    bool find(int value)
    {
        if(data==value)
            return true;
        if(right&&left)
            return right->find(value)||left->find(value);
        else if(right)
...
vector<long long>v;
long long counter=0;
void work1(long long three,long long  twos,long long  health,long long index)
{
    if(index==n)
        return;
    if(health>v[index])
    {
        work1(three,twos,health+v[index]/2,index+1);
        counter++;
    }
    else
    {
        if(twos>=1)
        {
            work1(three,twos-1,health*2,index);
        }
        else if(three)
        {
            work1(0,twos,health*3,index);
        }
        else
            return;
    }
}
void work2(long long three,long long twos,long long health,long long index)
{
    if(index==n)
        return;
    if(health>v[index])
    {
        work2(three,twos,health+v[index]/2,index+1);
        counter++;
    }
    else
    {
        if(three)
        {
            work2(0,twos,health*3,index);
        }
        else if(twos>=1)
        {
            work2(three,twos-1,health*2,index);
        }
        else
            return;
    }
}
void work3(long long three,long long twos,long long health,long long index)
{
    if(index==n)
        return;
    if(health>v[index])
    {
        work3(three,twos,health+v[index]/2,index+1);
        counter++;
    }
    else
    {
        if(twos>1)
        {
            work3(three,twos-1,health*2,index);
        }
        else if(three)
        {
            work3(0,twos,health*3,index);
        }
        else if(twos==1)
        {
            work3(three,0,health*2,index);
        }
        else
            return;
    }
