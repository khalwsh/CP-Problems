...
template<typename T>
void print_tree_posfix(tree_Node<T>*root)
{
    if(root==nullptr)
    {
        return;
    }
    print_tree_posfix(root->left);
    print_tree_posfix(root->right);
    cout<<root->data<<" ";
}
template<class T>
...
bool check_if_subsecquence(string &orginal,string &check,vector<int>&v,int mid);
...
int main() {
    ...
    string res;
    cin>>orginal>>res;
    vector<int>v((int)orginal.size());
    for(auto &val:v)
        cin>>val;
    int left=0,right=(int)orginal.size()-1;
    while(left<=right) {
        int mid=left+(right-left)/2;
        if (!check_if_subsecquence(orginal, res, v,mid))
            right=mid-1;
        else
            left=mid+1;
    }
...
bool check_if_subsecquence(string &orginal,string &check,vector<int>&v,int mid)
{
    ...
    for(int i=0;i<mid;i++)
        os.insert(v[i]);
    int counter=0;
    for(auto &val:orginal) {
        counter++;
        if(os.find(counter)!=os.end())
            continue;
        if (val == check[left])
              left++;
        if(left==(int)check.size())
            return true;
    }
    return false;
