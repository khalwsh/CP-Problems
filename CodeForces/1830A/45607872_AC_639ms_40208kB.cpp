    ...
    while(t--){
        ...
        while(!q.empty()){
            ...
            for(auto &val:adj[node]){
                if(mp[make_pair(parent,node)]<mp[make_pair(node,val)]&&dist[val]==inf){
                    dist[val]=dist[node];
                    q.push({val,node});
                }else if(dist[val]==inf){
                    dist[val]=dist[node]+1;
                    q.push({val,node});
                }
            }
        }
        assert(*max_element(dist.begin(),dist.end())!=inf);
        cout<<*max_element(dist.begin(),dist.end())+1<<line;//starting form 1;
    }
}