...
int main() {
    ...
    while(t--)
    {
        ...
        if(s.size()<tt.size())
            swap(tt,s);
        if(s!=tt&&(int)s.size()==tt.size())
            cout<<-1;
        else if(s==tt)
            cout<<s;
        else if(ss1.size()==1&&ss2.size()==1)
        {
            if(*ss1.begin()==*ss2.begin())
            {
                int factor=(int)s.size()*(int)tt.size()/__gcd((int)s.size(),(int)tt.size());
                string temp=tt;
                while(tt.size()!=factor)
                {
                    tt+=temp;
                }
                cout<<tt;
