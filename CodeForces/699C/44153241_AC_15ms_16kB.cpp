...
long long solve(int index,int last_taken,int n,vector<int>&v){
    if(index==n)return 0;
    if(dp[index][last_taken]!=-1)return dp[index][last_taken];
    if(last_taken==0){
        long long res=INT_MAX;
        if(v[index]==1)
            res=min(res, solve(index+1,1,n,v));
        if(v[index]==0)
            res=min(res,1+ solve(index+1,0,n,v));
        if(v[index]==2)
            res=min(res,solve(index+1,2,n,v));
        if(v[index]==3)
            res=min(res,min({solve(index+1,1,n,v), solve(index+1,2,n,v)}));
        return dp[index][last_taken]=res;
    }else if(last_taken==1){
        long long res=INT_MAX;
        if(v[index]==0)
            res=min(res,1+ solve(index+1,0,n,v));
        if(v[index]==2||v[index]==3)
            res=min(res,solve(index+1,2,n,v));
        if(v[index]==1)
            res=min(res,1+solve(index+1,0,n,v));
        return dp[index][last_taken]=res;
...
signed main() {
    ...
    dp.resize(n,vector<long long>(4,-1));
    cout<<solve(0,0,n,v)<<line;
}