...
/*----------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------*/
bool handle(int row,int col,vector<vector<char>>&v){
    int n=v.size();
    int m=v[0].size();
    if(valid(row+1,col,n,m)&&v[row+1][col]=='1'&&valid(row,col+1,n,m)&&v[row][col+1]=='1'&&valid(row+1,col+1,n,m)&&v[row+1][col+1]!='1')
        return false;//right down
    if(valid(row-1,col,n,m)&&v[row-1][col]=='1'&&valid(row,col+1,n,m)&&v[row][col+1]=='1'&&valid(row-1,col+1,n,m)&&v[row-1][col+1]!='1')
        return false;//right up
    if(valid(row+1,col,n,m)&&v[row+1][col]=='1'&&valid(row,col-1,n,m)&&v[row][col-1]=='1'&&valid(row+1,col-1,n,m)&&v[row+1][col-1]!='1')
        return false;//left down
    if(valid(row-1,col,n,m)&&v[row-1][col]=='1'&&valid(row,col-1,n,m)&&v[row][col-1]=='1'&&valid(row-1,col-1,n,m)&&v[row-1][col-1]!='1')
