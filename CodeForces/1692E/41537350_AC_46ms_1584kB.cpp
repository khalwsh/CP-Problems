...
int valid(vector<int>&pre,int mid,int i,int s)
{
    if(!i)
        return (pre[mid]==s)-(pre[mid]>s)+(pre[mid]<s);
    else
        return (pre[mid]-pre[i-1]==s)-(pre[mid]-pre[i-1]>s)+(pre[mid]-pre[i-1]<s);
}
int main() {
    khaled
    int t;
