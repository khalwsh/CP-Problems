...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
void PreparationforInternationalWomensDay (){
    int n;
    cin>>n;
    int k;
    cin>>k;
    ...
    vector<int>modul(v);
    for(auto &val:modul)val%=k;
    map<int,int>mp;
    for(auto &val:modul)mp[val]++;
    int res=0;
    for(int i=0;i<n;i++){
        if(k-modul[i]==modul[i]){
            if(mp[modul[i]]>=2){
                res+=2;
                mp[modul[i]]-=2;
            }
            ...
            if(modul[i]==0){
                if(mp[0]>=2)mp[0]-=2,res+=2;
            }else{
                if(mp[modul[i]]>=1&&mp[k-modul[i]]>=1){
                    mp[modul[i]]--;
                    mp[k-modul[i]]--;
                    res+=2;
                }
...
signed main() {
    khaled
    PreparationforInternationalWomensDay();
}