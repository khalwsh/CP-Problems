...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
bool can(vector<int>&towers,vector<int>&cities,int r){
    for(auto &val:cities){
        auto it= lower_bound(towers.begin(),towers.end(),val);
        if(it==towers.end()){
            it--;
            if(val<=*it+r&&val>=*it-r)continue;
            else return false;
            ...
            auto iit=it-1;
            if(val<=*it+r&&val>=*it-r)continue;
            else if(val<=*iit+r&&val>=*iit-r)continue;
            else return false;
        }
    }
    return true;
...
signed main() {
    ...
    for(auto &val:cities)cin>>val;
    for(auto &val:towers)cin>>val;
