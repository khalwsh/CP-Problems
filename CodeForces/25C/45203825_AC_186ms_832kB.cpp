    ...
    while(t--){
        ...
        while(k--){
            ...
            if(adj[a][b]>cost){
                ...
                for(int i=0;i<n;i++){
                    for(int j=0;j<=i;j++){
                        adj[i][j]=adj[j][i]=min(adj[i][j],adj[i][a]+adj[a][j]);
                        adj[i][j]=adj[j][i]=min(adj[i][j],adj[i][b]+adj[b][j]);
                        adj[i][j]=adj[j][i]=min(adj[i][j],adj[i][a]+adj[a][b]+adj[b][j]);
                        adj[i][j]=adj[j][i]=min(adj[i][j],adj[i][b]+adj[b][a]+adj[a][j]);
                    }
                }
            }
            int sum=0;
