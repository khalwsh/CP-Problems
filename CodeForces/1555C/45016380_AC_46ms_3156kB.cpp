...
signed main() {
    ...
    while(t--) {
        ...
        int tot=0;
        for(auto &val:v)for(auto &i:val)tot+=i;
        vector<int>pre(n),pre2(n);
        int res=INT_MAX;
        for(int i=0;i<n;i++)
            pre[i]+=((i==0)?0:pre[i-1])+v[0][i];
        for(int i=0;i<n;i++)
            pre2[i]+=((i==0)?0:pre2[i-1])+v[1][i];
        for(int i=0;i<n;i++){
            res=min(res,max(pre[n-1]-pre[i],(i==0?0:pre2[i-1])));
        }
        cout<<res<<line;
    }
}