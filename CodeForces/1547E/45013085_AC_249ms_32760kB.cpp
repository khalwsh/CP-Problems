...
signed main() {
    ...
    while(t--){
        ...
        vector<pair<int,int>>air(k);
        for(auto &val:air)cin>>val.first;
        for(auto &val:air)cin>>val.second;
        priority_queue<pair<int,int>,vector<pair<int,int>>,greater<>>pq;
        for(auto &val:air){
            pq.push({val.second,val.first-1}),res[val.first-1]=val.second;
        }
        while(!pq.empty()){
         ...
         int j=node+1;
         while(i>=0){
             if(res[i]>cost+abs(i-node)){
                 res[i]=cost+abs(i-node);
                 pq.push({res[i],i});
             }else
                 break;
             i--;
         ...
         while(j<res.size()){
             if(res[j]>cost+abs(j-node)){
                 res[j]=cost+abs(j-node);
                 pq.push({res[j],i});
             }else
                 break;
             j++;
