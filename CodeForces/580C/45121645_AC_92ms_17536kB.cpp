...
/*--------------------------------------------------------------------------------------------------------------------*/
int work=0;
void dfs(int node,int parent,vector<int>adj[],vector<int>&cats,int m,int in_way){
    if(cats[node])in_way++;
    else in_way=0;
    if(m+1<=in_way)return;
    if(adj[node].size()==1&&node!=0)work++;
    for(auto &val:adj[node]){
        if(val==parent)continue;
        dfs(val,node,adj,cats,m,in_way);
    }
}
signed main() {
    khaled
