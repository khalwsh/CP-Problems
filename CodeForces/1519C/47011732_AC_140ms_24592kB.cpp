...
signed main() {
    ...
    while(t--){
        int n;
        cin>>n;
        vector<int>ui(n),sk(n);
        set<int>senders;
        for(auto &val:ui)cin>>val,senders.insert(val);
        for(auto &val:sk)cin>>val;
        vector<int>adj[n+1];
        vector<int>pref[n+1];
        for(int i=0;i<n;i++){
            adj[ui[i]].emplace_back(sk[i]);
            pref[ui[i]].emplace_back(sk[i]);
        }
        for(int i=1;i<=n;i++){
        ...
        for(int i=1;i<=n;i++){
            ...
            int k=i;
            int sum=0;
            for(auto &val:senders){
                if(adj[val].size()<k) {
                    del.emplace_back(val);
                    continue;
                }
                sum+= query(0,(int)pref[val].size()/k*k-1,pref[val]);
            }
            cout<<sum<<" ";
            for(auto &val:del)
                senders.erase(val);
