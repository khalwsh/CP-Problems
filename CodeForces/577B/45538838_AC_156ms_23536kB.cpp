...
/*--------------------------------------------------------------------------------------------------------------------*/
int dp[1001][1001][2];
bool solve(int index,vector<int>&v,int m,int sum,int first_call){
    if(index==v.size()){
        return sum%m==0&&first_call;
    }
    if(~dp[index][sum][first_call])return dp[index][sum][first_call];
    bool leave=solve(index+1,v,m,sum,first_call);
    bool take=solve(index+1,v,m,add(sum,v[index],m),true);
    return dp[index][sum][first_call]= leave||take;
}
signed main() {
    khaled
    int n,m;
    ...
    cout<<(solve(0,v,m,0,false)?"YES":"NO");
}