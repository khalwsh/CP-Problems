...
string s;
int dp[N][3];
int solve(int index,int cur_sum){
    if(index==s.size()){
        return 0;
    }
    int &ret=dp[index][cur_sum];
    if(~ret)return ret;
    cur_sum=add(cur_sum,s[index]-'0',3);
    //stop cutting here and start new cut or continue cutting
    int choice1=solve(index+1,0)+(cur_sum==0);
    int choice2=solve(index+1,cur_sum);
