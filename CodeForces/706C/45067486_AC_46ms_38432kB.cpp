...
vector<int> v;
vector<string> s;
int solve(int i , bool changed) {
    if(i == n) return 0;

    int &ret = dp[i][changed];
    if(~ret) return ret;
    ...
    if(i == 0) {
        ...
        ret = v[i] + solve(i + 1 , 1);
        // not reverse
        ret = min(ret , solve(i + 1 , changed));
    }
    else {
        string prev = s[i - 1];
        if(changed) reverse(prev.begin() , prev.end());
        // not reverse
        if(prev <= s[i]) ret = min(ret , solve(i + 1 , 0));
        // reverse
        string cur = s[i];
        reverse(cur.begin() , cur.end());
        if(prev <= cur) ret = min(ret , v[i] + solve(i + 1 , 1));
    }
    return ret;
}
signed main() {
    ...
    int ret=solve(0,false);
    cout<<(ret==inf?-1:ret)<<line;
}