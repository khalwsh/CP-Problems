...
vector<long long> divisors(long long n)
{
    ...
    for(long long i=2;i*i<=n;i++)
    {
        ...
        {
            if(n/i!=i)
                dev.emplace_back(i),dev.emplace_back(n/i);
            else
                dev.emplace_back(i);
        }
    }
...
long long binary_search_khaled(long long lower,long long upper,vector<long long>&common)
{
    long long left=0,right=common.size()-1;
    long long res=-1;
    while(left<=right)
    {
        long long mid=left+(right-left)/2;
        if(common[mid]>=lower&&common[mid]<=upper)
        {
            res=common[mid];
            left=mid+1;
        }
        else if(common[mid]>upper)
            right=mid-1;
        else
            left=mid+1;
    }
...
int main() {
    ...
    long long a,b;
    cin>>a>>b;
    vector<long long>a_divisor= divisors(a);
    vector<long long>b_divisor= divisors(b);
    map<long long,int>mp;
    for(auto &val:a_divisor)
        mp[val]++;
    for(auto &val:b_divisor)
        mp[val]++;
    vector<long long>common;
    for(auto &val:mp)
    {
        if(val.second==2)
            common.emplace_back(val.first);
    }
    ...
    while(n--)
    {
       ...
       cin>>left>>right;
       cout<<binary_search_khaled(left,right,common);
       cout<<line;
    }
 }