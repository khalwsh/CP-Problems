...
int vis[19][4][2];
int tx=1;
int solve(int index,string&s,int non_zero,bool tight){
    if(non_zero>=4)return 0;
    if(index==s.size()){
        return non_zero<4;
    }
    int &ret=dp[index][non_zero][tight];
    if(vis[index][non_zero][tight]==tx)return ret;
    vis[index][non_zero][tight]=tx;
    int lim=(tight?9:s[index]-'0');
    int res=0;
    for(int i=0;i<=lim;i++){
        res+=solve(index+1,s,non_zero+(i!=0),tight||s[index]-'0'>i);
    }
    return ret =  res;
}
signed main() {
    ...
    while(t--){
      ...
      tx++;
      int res=solve(0,b,0,false);
      tx++;
      res-=solve(0,a,0,false);
      tx++;
      cout<<res<<line;
    }
}