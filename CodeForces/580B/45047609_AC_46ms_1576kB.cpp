...
long long find_summation(long long start,long long finish,vector<pair<long long,long long>>&vp);
int main() {
    khaled
    int n;
    cin>>n;
    int d;
    cin>>d;//amout of money make them feel poor
    ...
    long long poor=0,rich=0;
    long long my_max=0;
    vector<pair<long long,long long>>vv;
    int counter=0;
    for(int i=0;i<n;i++)
    {
        while(vp[rich].first-vp[poor].first>=d)
        {
            if(counter==0)
                vv.emplace_back(poor,rich);
           poor++;
           counter++;
        }
        rich++;
    ...
    for(auto &val:vv)
    {
        my_max=max(my_max,find_summation(val.first,val.second,vp));
    }
    cout<<my_max;
}
long long find_summation(long long start,long long finish,vector<pair<long long,long long>>&vp)
