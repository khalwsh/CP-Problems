...
int solve(int index,string &b,string&a,int m,int d,int multi,bool tight,bool down){
    if(index==b.size()){
        return multi%m==0;
    }
    int &ret=dp[index][multi][tight][down];
    if(~ret)return ret;
    if(index&1){
        if(d>b[index]-'0'&&!tight)return ret = 0;
        if(d<a[index]-'0'&&!down) return ret = 0;
        return ret = solve(index+1,b,a,m,d,add(multi,mul(powers[index],d,m),m),tight||d<b[index]-'0',down||d>a[index]-'0')%mod;
    }
    int low=(down?0:a[index]-'0');
    int lim=(tight?9:b[index]-'0');
    int res=0;
    for(int i=low;i<=lim;i++){
        if(i==d)continue;
        res=add(res,solve(index+1,b,a,m,d,add(multi,mul(powers[index],i,m),m),tight||i<b[index]-'0',down||i>a[index]-'0'),mod);
    }
    return ret = res;
}
signed main() {
    ...
    string a,b;
    cin>>m>>d>>a>>b;
    powers.resize(a.size(),0);
    powers[0]=1;
    for(int i=1;i<a.size();i++){
        powers[i]=mul(powers[i-1],10ll,m);
    }
    ...
    int i=0;
    for(auto &val:a){
        s=add(s, mul(powers[i++],val-'0',m),m);
    }
    bool found=true;
    ...
    int res=solve(0,b,a,m,d,0,false,false);
    cout<<res<<line;
}