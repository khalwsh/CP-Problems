...
int n,m;
vector<string>temp;
set<vector<string>>all_sol;
void generate_all_possible_combinations(int index)
{
    if(index==n)
    {
        all_sol.insert(temp);
    ...
    generate_all_possible_combinations(index+1);
    temp.pop_back();
    generate_all_possible_combinations(index+1);
}
int main() {
    ...
    generate_all_possible_combinations(0);
    vector<string>res;
    for(auto &val: all_sol)
    {
        bool flag=true;
        int counter=0;
        for(auto &i:val)
        {
            for(int l=counter+1;l<val.size();l++)
            {
                if(hate[i].find(val[l])==hate[i].end())
                    continue;
                else
                {
                    flag= false;
