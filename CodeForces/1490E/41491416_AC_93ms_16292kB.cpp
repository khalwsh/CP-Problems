...
bool binary_sea(vector<long long>&pre,vector<long long>&v,long long &my_max,int& n,int index)
{
...
int main() {
    ...
    while(t--)
    {
        ...
        int counter=1;
        for(auto &val:v)
            cin>>val,mp[counter++]=val;
        sort(v.begin(),v.end());
        long long my_max=v[n-1];
        vector<long long>prefix(n);
        prefix[0]=v[0];
        for(int i=1;i<n;i++)
            prefix[i]=prefix[i-1]+v[i];
        map<long long,bool>res;
        for(int i=0;i<n;i++)
        {
            if(binary_sea(prefix,v,my_max,n,i))
                res[v[i]]=true;
        }
        ...
        {
            if(res[val.second])
                final_res.emplace_back(val.first);
        }
        cout<<(int)final_res.size()<<line;
        sort(final_res.begin(),final_res.end());
        for(auto &val:final_res)
            cout<<val<<" ";
        cout<<line;
    }
}