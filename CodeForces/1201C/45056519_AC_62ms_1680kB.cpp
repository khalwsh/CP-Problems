...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
bool can1(int days,vector<int>&a,vector<int>&b,int n){
    int use1=days-days/4;
    int use2=use1;
    int alice=0;
    if(days>=n){
        alice+=(days-n)*100;
...
void CountKangrooIsFun(){
    ...
    while(!ms.empty()){
        int val1=*ms.begin();
        int low=val1/2;
        if(ms.lower_bound(low)!=ms.end()){
            ms.erase(ms.lower_bound(low));
        }
...
bool can2(int mid,vector<int>&v,int k){
    int n=(int)v.size();
    int res=0;
    for(int i=n/2;i<n;i++){
        if(v[i]<mid){
