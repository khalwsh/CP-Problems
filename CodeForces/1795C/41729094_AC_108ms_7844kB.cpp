...
int main() {
    ...
    int t;
    cin >> t;
    /* I made a prefix sum on testers and binary search at each sort of tea 
     * so that i know all which will take their min and the one who will have the reminder
     * then mark the first of the range will have their limits and the end by -1
     * so when finsih I just make perfix sum on the array mark 
     * then add mark*limit to the result
     */
     while (t--) {
        int n;
        cin >> n;
        vector<long long> tea(n), testers(n);
        for (auto& x : tea)
            cin >> x;
        for (auto& x : testers)
            cin >> x;
        ...
        for (int i = 0; i < n; ++i)
            sum[i + 1] = sum[i] + testers[i];
        vector<long long> mark(n + 1), add(n + 1);
        for (int i = 0; i < n; ++i) {
            int j =(int)(upper_bound(sum.begin(), sum.end(), tea[i] + sum[i]) - sum.begin() - 1);
            mark[i] += 1;
            mark[j] -= 1;
            add[j] += tea[i] - (sum[j] - sum[i]);
        }
        for (int i = 0; i < n; ++i) {
            cout << mark[i] * testers[i] + add[i] << ' ';
            mark[i + 1] += mark[i];
        }
        cout << line;
    }
