...
void build(int ind,int n){
    for(int i=1;i<=kk;i++){
        for(int j=0;j+(1<<i)<=n;j++){
            sp[ind][i][j]=max(sp[ind][i-1][j],sp[ind][i-1][j+(1<<(i-1))]);
        }
    }
}
int query(int ind,int l,int r){
    int lg= log2_floor(r-l+1);
    return max(sp[ind][lg][l],sp[ind][lg][r-(1<<lg)+1]);
}
signed main() {
    khaled
    int n,m,k;
    cin>>n>>m>>k;
    sp.resize(m,vector<vector<int>>(kk+1,vector<int>(n)));
    ...
    for(int i=0;i<m;i++){
        std::copy(v[i].begin(),v[i].end(),sp[i][0].begin());
        build(i,n);
    }
    int res=0;
    vector<int>ms;
    for(int i=0;i<n;i++){
        ...
        while(left<=right){
            ...
            if(sum<=k){
                ...
                temp_ans.clear();
                for(int j=0;j<m;j++){
                    temp_ans.emplace_back(query(j,i,mid));
                }
            }else{
                right=mid-1;
            }
    ...
    while(ms.size()!=m)ms.emplace_back(0);
