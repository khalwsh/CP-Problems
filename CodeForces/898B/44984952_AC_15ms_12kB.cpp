...
int binary1(int x,int k,int b){
    int left=1,right=1e17;
    int res=1;
    while(left<=right){
        int mid=left+(right-left)/2;
        if(x+mid*b/k>=0)res=mid,right=mid-1;
        else left=mid+1;
    }
    return res;
}
int binary2(int y,int k,int a){
    int left=(int)-inf,right=1;
    int res=1;
    while(left<=right){
        int mid=left+(right-left)/2;
        if(y-mid*a/k>=0)res=mid,left=mid+1;
        else right=mid-1;
    }
    return res;
}
...
//    cout<<temp1<<" "<<temp2<<line;
    //x--->x+(factor*b/gcd(a,b)),y-(factor*a/gcd(a,b)))
    int factor=0;
    if(x<0){
        factor=binary1(x,k,b);
    }
    if(y<0) {
        factor = binary2(y, k, a);
    }
//    long long to=factor;
//    cout<<to;
    x=x+factor*b/k,y=y-factor*a/k;
    long long res1=x,res2=y;
    //cout<<res1<<" "<<res2<<line;
    if(x<0||y<0)
        cout<<"NO";
    else
        cout<<"YES"<<line<<res1<<" "<<res2<<line;
}