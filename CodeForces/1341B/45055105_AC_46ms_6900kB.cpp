...
signed main() {
    ...
    while(t--){
        ...
        set<int>peaks;
        for(int i=1;i<n-1;i++){
            pre[i]=(v[i]>v[i-1]&&v[i]>v[i+1]);
            if(pre[i])peaks.insert(i);
        }
        for(int i=1;i<n;i++)
            pre[i]+=pre[i-1];
        int mx=0,pos=0;
        for(int i=0;i+k-1<n;i++){
            if(peaks.count(i)){
                if(peaks.count(i+k-1)){
                    if(pre[i+k-1]-(i==0?0:pre[i])-2>mx)pos=i;
                    mx=max(pre[i+k-1]-(i==0?0:pre[i])-2,mx);
                }else {
                    if (pre[i + k - 1] - (i == 0 ? 0 : pre[i]) - 1 > mx)pos = i;
                    mx = max(pre[i + k - 1] - (i == 0 ? 0 : pre[i]) - 1, mx);
                }
            }else if(peaks.count(i+k-1)){
                if(pre[i+k-1]-(i==0?0:pre[i])-1>mx)pos=i;
                mx=max(pre[i+k-1]-(i==0?0:pre[i])-1,mx);
            }else {
                if (pre[i + k - 1] - (i == 0 ? 0 : pre[i]) > mx)pos = i;
                mx = max(pre[i + k - 1] - (i == 0 ? 0 : pre[i]), mx);
            }
        }
        cout<<mx+1<<" "<<pos+1<<line;
    }
}