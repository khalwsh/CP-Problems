...
class ds{
public:
    //linked list queue stack binary search tree
    //binary search linear search sorting
    ...
    private:
        void reverseRecursive(linked_list_node* prev, linked_list_node* cur) {
            if(cur==nullptr)
                return;
            if(cur->next==nullptr)
            {
...
bool reverse_sort(vector<int>&v,int n)
{
    vector<int>k(v);
...
int get_max_left(int index,vector<int>&v)
{
    int fixed=v[index];
...
int get_max_right(int index,vector<int>&v)
{
    int fixed=v[index];
...
int get_min_right(int index,vector<int>&v)
{
    int fixed=v[index];
    for(int i=index+1;i<(int)v.size();i++)
    {
...
int get_min_left(int index,vector<int>&v)
{
    int fixed=v[index];
    for(int i=0;i<index;i++)
    {
...
int main() {
    ...
    cin>>n;
    vector<int>v(n);
    vector<bool>smaller_left(n);
    vector<bool>smaller_right(n);
    vector<bool>bigger_left(n);
    vector<bool>bigger_right(n);
    int my_min=INT_MAX;
    int my_max=INT_MIN;
    for(int i=0;i<n;i++)
    {
     ...
     for(int i=n-1;i>=0;i--)
     {
         ...
         my_max=max(my_max,v[i]);
         if(my_min==v[i])
             smaller_right[i]=false;
         else
             smaller_right[i]=true;
         if(my_max==v[i])
             bigger_right[i]=false;
         else
             bigger_right[i]=true;
     }
    if(sorted(v,n)|| reverse_sort(v,n))
    {
        cout<<0<<line;
    }
    else
    {
        ...
        for(int i=1;i<n-1;i++)
        {
            if(smaller_left[i]&&smaller_right[i]||bigger_right[i]&&bigger_left[i]) {
                pos1 = get_max_left(i, v);
                pos2 = get_max_right(i, v);
                if (pos1 != -1 && pos2 != -1) {
                    pos3 = i;
                    break;
                }
                pos1 = get_min_left(i, v);
                pos2 = get_min_right(i, v);
                if (pos1 != -1 && pos2 != -1) {
                    pos3 = i;
                    break;
                }
        ...
        vector<int>pos(3);
        pos[0]=pos1;
        pos[1]=pos2,pos[2]=pos3;
        cout<<3<<line;
        sort(pos.begin(),pos.end());
        cout<<pos[0]+1<<" "<<pos[1]+1<<" "<<pos[2]+1<<line;
    }
