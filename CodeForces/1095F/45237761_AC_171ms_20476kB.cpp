...
signed main() {
    ...
    for(int i=0;i<m;i++){
        ...
        cin>>a>>b>>cost;
        a--,b--;
        edges.emplace_back(cost, make_pair(a,b));
    }
    int idx= min_element(v.begin(),v.end())-v.begin();
    for(int i=0;i<n;i++){
        if(i==idx)continue;
        edges.emplace_back(v[i]+v[idx], make_pair(idx,i));
    }
    sort(edges.begin(),edges.end());
    int sum=0;
    dsu d(n);
    for(auto &val:edges){
        if(d.find_parent(val.second.second)==d.find_parent(val.second.first))continue;
        d.union_by_size(val.second.second,val.second.first);
        sum+=val.first;
    }
    cout<<sum<<line;
}