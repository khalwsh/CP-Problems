...
int main() {
    ...
    while(t--)
    {
        ...
        while(right!=n)
        {
            ...
            if(ss.size()==3)
            {
               min_distant=min(min_distant,(int)ms.size());
               while(left!=right-1)
               {
                  ms.erase(ms.find(s[left]));
                  if(ms.count(s[left])==0) {
                      min_distant=min(min_distant,right-left+1);
                      ss.erase(s[left]);
                      left++;
                      break;
                  }
        ...
        if(min_distant==INT_MAX)
            min_distant=0;
        cout<<min_distant<<line;
    }
}