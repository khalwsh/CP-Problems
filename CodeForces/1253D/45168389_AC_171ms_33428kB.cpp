...
signed main() {
    ...
    for(int i=0;i<n;i++){
        mp[d.find(i)].emplace_back(i);
    }
    vector<vector<int>>comps;
    for(auto &val:mp)
        comps.emplace_back(val.second);
    for(auto &val:comps)sort(val.begin(),val.end());
    int res=0;
    sort(comps.begin(),comps.end());
    int mx=comps.front().front();
    for(auto &val:comps){
        if(val.size()<=1)continue;
        mx=max(mx,val.front());
        for(int i=mx;i<=val.back();i++){
            if(d.find(i)!=d.find(mx))d.merge(i,mx),res++;
        }
        mx=max(mx,val.back());
    }
    cout<<res<<line;
