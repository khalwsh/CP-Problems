...
long long solve(int index,int last_taken,vector<int>&intial_colors,int n,int m,int k,vector<vector<int>>&cost){
    if(index==n){
        if(k==0)return 0;
        return 1e16;
    }
    if(k<0)return 1e16;
    int &ret=dp[index][last_taken][k];
    if(~ret)return ret;
    if(intial_colors[index]!=0){
        if(last_taken==intial_colors[index])return ret=solve(index+1,last_taken,intial_colors,n,m,k,cost);
        else return ret=solve(index+1,intial_colors[index],intial_colors,n,m,k-1,cost);
    }else{
        int res=1e16;
        for(int i=1;i<=m;i++){
            if(last_taken==i){
                res=min(res, solve(index+1,last_taken,intial_colors,n,m,k,cost)+cost[index][i-1]);
            }else{
                res=min(res,solve(index+1,i,intial_colors,n,m,k-1,cost)+cost[index][i-1]);
            }
        }
        return ret=res;
    }
...
signed main() {
    ...
    int n,m,k;
    cin>>n>>m>>k;
    vector<int>intial_colors(n);
    dp.resize(n+1,vector<vector<int>>(m+2,vector<int>(k+1,-1)));
    for(auto &val:intial_colors)cin>>val;
    vector<vector<int>>cost(n,vector<int>(m));
    for(auto &val:cost)for(auto &i:val)cin>>i;
    int res= solve(0,m+1,intial_colors,n,m,k,cost);
    cout<<(res>=1e16?-1:res)<<line;
}