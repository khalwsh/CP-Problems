...
/*--------------------------------------------------------------------------------------------------------------------*/
int dp[11][6][6];
int solve(int index,string&s,int score1,int score2){
    if(index==s.size()){
        return 0;
    }
    int rem_first_team=(int)(10-index)/2+(index&1^1);
    int rem_second_team=(int)(10-index)/2+(index&1);
    if(rem_first_team+score1<score2||rem_second_team+score2<score1)return 0;
    int &ret=dp[index][score1][score2];
    if(~ret)return ret;
    if(index&1^1){
        if(s[index]=='1')return  ret=1+solve(index+1,s,score1+1,score2);
        else if(s[index]=='0')return ret=1+solve(index+1,s,score1,score2);
        else return ret=1+min({solve(index+1,s,score1+1,score2),solve(index+1,s,score1,score2)});
    }else{
        if(s[index]=='1')return ret=1+solve(index+1,s,score1,1+score2);
        else if(s[index]=='0')return ret=1+solve(index+1,s,score1,score2);
        else return ret=1+min({solve(index+1,s,score1,1+score2),solve(index+1,s,score1,score2)});
