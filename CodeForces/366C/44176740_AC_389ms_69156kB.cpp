...
long long solve(int index,int balance,vector<int>&taste,vector<int>&calories){
        if(index==n){
            if(balance==0)return 0;
            else return INT_MIN;
        }
        if(dp[index].count(balance))return dp[index][balance];
        long long res1=solve(index+1,balance,taste,calories);
        long long res2=solve(index+1,balance+(taste[index]-calories[index]),taste,calories)+taste[index];
        return dp[index][balance]=max(res1,res2);
}
signed main() {
    khaled
    cin>>n>>k;
    vector<int>taste(n),calories(n);
    for(auto &val:taste)cin>>val;
    for(auto &val:calories)cin>>val,val*=k;
    dp.resize(n);
    int res=solve(0,0,taste,calories);
    cout<<(res<=0?-1:res)<<line;
}