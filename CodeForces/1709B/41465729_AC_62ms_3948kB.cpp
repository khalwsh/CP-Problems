...
int main() {
    ...
    suffix_sum[n-1]=suffix[n-1];
    for(int i=n-2;i>=0;i--)
    {
        suffix_sum[i]=suffix_sum[i+1]+suffix[i];
    }
    //cout<<line;
    prefix_sum[0]=pre[0];
    for(int i=1;i<n;i++)
        prefix_sum[i]=prefix_sum[i-1]+pre[i];
    while(q--)
    {
        int left,right;
        cin>>left>>right;
        ...
        right--;
        if(left==0&&right!=0)
            cout<<prefix_sum[right-1];
        else if(right==0&&left==0||left==right)
            cout<<0;
        else if(left>right)
        {
            if(right==0)
            {
                if(left!=n-1)
                   cout<<abs(suffix_sum[left+1]-suffix_sum[0]);
                else
                    cout<<suffix_sum[0];
            }
            else
            {
                cout<<abs(suffix_sum[right+1]-suffix_sum[left+1]);
            }
        }
        else
            cout<<abs(prefix_sum[right-1]-prefix_sum[left-1]);
        cout<<line;
    }
