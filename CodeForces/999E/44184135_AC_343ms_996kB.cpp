...
void dfs(int node,vector<bool>&vis,vector<int>adj[])
{
    ...
    {
        if(!vis[val])good[val]=true,dfs(val,vis,adj);
    }
}
void dfs2(int node,vector<int>adj[],vector<bool>&vis)
{
    ...
    for(auto &val:adj[node])
    {
        if(!vis[val])counter++,dfs2(val,adj,vis);
    }
}
signed main() {
    khaled
    ...
    for(int i=0;i<n;i++)
    {
        ...
        {
            counter=0;
            vector<bool>vevo(n,false);
            counter++;
            dfs2(i,adj,vevo);
            vp.emplace_back(counter,i);
        }
    }
